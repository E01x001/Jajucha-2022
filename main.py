...
class Planning(BasePlanning):
    def __init__(self, graphics):
        ...
        self.vars.steer = 5  # 조향값은 0인 상태로 변수 설정
        self.vars.velocity = 20  # 속도도 0인 상태로 변수 설정
        self.vars.noLine = 0
    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        ##print(ROI)
        lines_j = self.for_ROI(frontImage)
        leftiest = 0
        rightiest = 700
        myline = [[], []]
        if (len(lines_j) != 0) :
            for i in lines_j :
                for j in i :
                    tangent = (j[3] - j[1]) / (j[2] - j[0])
                    avg = (j[0] + j[2]) / 2
                    if (avg > leftiest and tangent < -0.2) :
                        myline[0] = j
                        leftiest = avg
                    elif (avg < rightiest and tangent > 0.2) :
                        myline[1] = j
                        rightiest = avg
        else : print("라인 없음")
        ...
        for i in range(0, 2, 1) :
            if (len(myline[i]) != 0) :
                proper_tangent[i] = np.abs((myline[i][3] - myline[i][1]) / (myline[i][2] - myline[i][0]))
            else :
                proper_tangent[i] = -1
        ...
        # else:
        #     return self.vars.steer, self.vars.velocity
        self.vars.steer, self.vars.velocity = self.get_steer_from_tangent(proper_tangent, bias)
        print(self.vars.steer)
        return self.vars.steer, self.vars.velocity
        

    def get_steer_from_tangent(self, tangent, bias) :
        steer = 0
        if (tangent[0] == -1 and tangent[1] == -1) :
            return self.vars.steer, self.vars.velocity

        if (tangent[0] == -1) :
            steer = -np.abs(pow(10, np.abs(0.65 - tangent[1])) - 1) * 35 
            if (steer < -100) : steer = -100

        elif (tangent[1] == -1) :
            steer = np.abs(pow(10, np.abs(0.8 - tangent[0])) - 1) * 35
            if (steer > 100) : steer = 100

        self.vars.steer = steer - 17 + bias * 0.1
        return self.vars.steer, self.vars.velocity
